...
const billsEmail = bills.map((bill) => {
  return {
    ...bill,
...
describe("Given I am connected as an Admin", () => {
  ...
  // email include first and last name
  describe("When I am on Dashboard page", () => {
    test("Then, first and last name should be splited", () => {
      const firstBill = billsEmail[0];
      const html = card(firstBill);
      document.body.innerHTML = html;
      const nameFromEmail = screen.getByText("john doe");
      expect(nameFromEmail).toBeTruthy();
    });
  });
  ...
  describe("When I am on Dashboard page and I click on arrow", () => {
    ...
    // Card should open
    test("Then, tickets list should be unfolding, and card should open", () => {
      const onNavigate = (pathname) => {
        document.body.innerHTML = ROUTES({ pathname });
      };
      ...
      dashboard.handleShowTickets(null, bills, 2);

      const billId = "UIUZtnPQvnbFnB0ozvJh";

      const billCard = screen.getByTestId(`open-bill${billId}`);
      const handleEditTicket = jest.fn();
      dashboard.handleEditTicket = handleEditTicket;
      billCard.addEventListener("click", handleEditTicket);

      userEvent.click(billCard);
